#pragma once
//log文件路径  
#define LOG_FILE_NAME_ "data_log.txt"  
//启用开关  
#define LOG_ENABLE_  
#include <tchar.h>
#include <fstream>    
...
//用下面这些宏来使用本文件  
#define LOG_(x)              CLog_::WriteLog(x);          //括号内可以是字符串(ascii)、整数、浮点数、bool等  
#define LOG2_(x1,x2)     CLog_::WriteLog2(x1,x2);  
#define LOG_FUNC_        LOG_(__FUNCTION__)               //输出当前所在函数名  
#define LOG_LINE_        LOG_(__LINE__)                       //输出当前行号  
#define LOG_FUNC_BEGIN_  CLog_::WriteFuncBegin(__FUNCTION__);     //形式如：[时间]"------------FuncName  Begin------------"  
#define LOG_FUNC_END_     CLog_::WriteFuncEnd(__FUNCTION__);      //形式如：[时间]"------------FuncName  End------------"  
...
#define LOG_FUNC_          
#define LOG_LINE_          
#define LOG_FUNC_BEGIN_    
#define LOG_FUNC_END_      
