...
LRESULT CMainFrame::OnMyChange(WPARAM wParam, LPARAM lParam)
{
	...
	{
		//CUserDlg类需要包含头文件#include "UserDlg.h"
		Context.m_pNewViewClass = RUNTIME_CLASS(CSpecialContractDialog);
		Context.m_pCurrentFrame = this;
		Context.m_pLastView = (CFormView*)m_spliter.GetPane(0, 1);
		m_spliter.DeleteView(0, 1);
		m_spliter.CreateView(0, 1, RUNTIME_CLASS(CSpecialContractDialog), CSize(1000, 600), &Context);
		CSpecialContractDialog* pNewView = (CSpecialContractDialog*)m_spliter.GetPane(0, 1);
		m_spliter.RecalcLayout();
		pNewView->OnInitialUpdate();
		m_spliter.SetActivePane(0, 1);
	}
